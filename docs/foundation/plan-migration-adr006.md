...
- **`format` et `statutDiffusion`** — résolus par ADR-007 : préservés dans `metadonneesMigration` (JSON), un champ explicitement temporaire. Échéance de revue fixée par ADR-007 : au plus tard à l'ouverture de la Phase 3 (Livrable 11) — pas de délai flottant.
