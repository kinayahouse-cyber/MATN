...
export function BriefEditor({
  ...
  const setType = (id: string, type: BriefBlockType) =>
    setBlocks((bs) => {
        ...
        {blocks.map((block) => (
          ...
          >
            ...
            <textarea
              ...
              onChange={(e) => {
                setText(block.id, e.target.value);
                e.target.style.height = 'auto';
                e.target.style.height = `${e.target.scrollHeight}px`;
              }}
              onBlur={() => persist(blocks)}
              onKeyDown={(e) => {
              ...
              className={`w-full resize-none overflow-hidden rounded-md border border-transparent bg-transparent px-2 py-1 placeholder:text-muted/50 focus:border-line focus:outline-none ${BLOCK_CLASS[block.type]}`}
            />
          </div>
        ))}
      </div>
